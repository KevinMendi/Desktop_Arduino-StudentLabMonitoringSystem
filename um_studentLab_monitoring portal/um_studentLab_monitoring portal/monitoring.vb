        ...
        Try
            ...
            Dim qry As String = "select s2.sbjctcode, s2.sbjctname, s2.type, s2.time, s1.name from tbl_subject t1 " & _
                             "left join mst_teacher s1 " & _
                                ...
                                "where t1.teacherid='" & teacherVerification.txtTeacherID.Text & "'"
...
#End Region
    ...
    Private Sub monitoring_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        teacherVerification.ShowDialog()
        _checkTeacher()
    ...
    Public Sub _viewDetails()
        Try
            ...
            Dim qry As String = "select s1.* from mst_student s1 " & _
                                ...
                                "where s1.rfid = '" & tagNumber.Text & "' and s2.tblsbjctid='" & lbCode.Text & "' and s1.tag='0'"
            Dim dr As MySqlDataReader
            Dim adapt As New MySqlDataAdapter
            Dim cmd As New MySqlCommand
...
#Region "mybless"
    Public Sub _something()
        Try
            ...
            Dim qry As String = "select * from login where studentid='" & idNo.Text & "' OR tblsbjctid='" & lbCode.Text & "'"
            Dim dr As MySqlDataReader
            Dim adapt As New MySqlDataAdapter
            Dim cmd As New MySqlCommand
...
#Region "SAVE LOG DATA"
    Public Sub _saveLogData()
        Try
            ...
            Dim cb As String = "INSERT INTO login( studentid, 	tblsbjctid, timeIn, dateIn)" & _
...
#Region "UPDATE STATUS"
    Public Sub _updateStatus()
        Try
            ...
            Dim qry As String = " UPDATE mst_student SET  tag='1'  where rfid=@rfid"
